...
class Equation:
    def __init__(self, eq: str) -> None:
        ...
        self.rslope = ('undef' if 'x=' in self.eq else
                       0 if 'x' not in self.eq else None)
...
class AllPoints:
    ...
    def in_points() -> list[Point]:
        ...
        for key, str_p in enumerate(pl):
            ...
            pl[key] = Point(m.group('x'), m.group('y'), m.group('n') if m.group('n') else '')
        return pl

    def translate_all(self, shift: Point) -> None:
        print('translations:')
        for key, pt in enumerate(self.allpoints):
            self.allpoints[key].x, self.allpoints[key].y = pt.x + shift.x, pt.y + shift.y
    ...
    def dilate_all(self, args: tuple[Point, float]) -> None:
        ...
        for pt in self.allpoints:
            pt.x, pt.y = (point_of_dil.x + scale * (pt.x - point_of_dil.x),
...
class Inputs:
        ...
        while True:
            ...
            if len(this_oper) != 1:
                ...
                for t in match_list:
                    ...
                    yield [name_dict[t[0]], type_dict[t[0]](*func_args)]
            else:
                this_args = self.in_val(this_oper)
                func_args = self.rematch(this_args, this_oper)
                yield [name_dict[this_oper[0]], type_dict[this_oper[0]](*func_args)]
    ...
    @staticmethod
    def rematch(s, m) -> tuple[str, ...]:
        match m:
            case 'T':
                return search(r'([\d.-]+)[, ]+([\d.-]+)', s).groups()
            case 'r':
                return search(r'[\dy=x.+-]+', s).group(),
            case _:
                return search(r'([\d.-]+)[, ]+([\d.-]+)[, ]+([\d.-]+)', s).groups()
